...
# Get the time measured by ultrasonic #1 in us (0 for no detection)
COMMAND_GET_TIME_USM1 = 1
# Get the time measured by ultrasonic #2 in us (0 for no detection)
COMMAND_GET_TIME_USM2 = 2
# Get the time measured by ultrasonic #3 in us (0 for no detection)
COMMAND_GET_TIME_USM3 = 3
# Get the time measured by ultrasonic #4 in us (0 for no detection)
COMMAND_GET_TIME_USM4 = 4
...
# Set the PWM duty cycle for drive #1 (16 bit, ignores limit checks)
COMMAND_CALIBRATE_PWM1 = 13
# Set the PWM duty cycle for drive #2 (16 bit, ignores limit checks)
COMMAND_CALIBRATE_PWM2 = 14
# Set the PWM duty cycle for drive #3 (16 bit, ignores limit checks)
COMMAND_CALIBRATE_PWM3 = 15
# Set the PWM duty cycle for drive #4 (16 bit, ignores limit checks)
COMMAND_CALIBRATE_PWM4 = 16
...
# Get the filtered time measured by ultrasonic #1 in us (0 for no detection)
COMMAND_GET_FILTER_USM1 = 41
# Get the filtered time measured by ultrasonic #2 in us (0 for no detection)
COMMAND_GET_FILTER_USM2 = 42
# Get the filtered time measured by ultrasonic #3 in us (0 for no detection)
COMMAND_GET_FILTER_USM3 = 43
# Get the filtered time measured by ultrasonic #4 in us (0 for no detection)
COMMAND_GET_FILTER_USM4 = 44
...
The busNumber if supplied is which I�C bus to scan, 0 for Rev 1 boards, 1 for Rev 2 boards, if not supplied the default is 1
    ...
    for address in range(0x03, 0x78, 1):
        try:
            ...
            pass
    if len(found) == 0:
        print('No UltraBorg boards found, is bus #%d correct (should be 0 for Rev 1, 1 for Rev 2)' % (
            busNumber))
    elif len(found) == 1:
    ...
    try:
        ...
        if len(i2cRecv) == I2C_MAX_LEN:
            if i2cRecv[1] == I2C_ID_SERVO_USM:
                ...
                foundChip = False
                print('Found a device at %02X, but it is not a UltraBorg (ID %02X instead of %02X)' % (
...
If tryOtherBus is True, this function will attempt to use the other bus if the ThunderBorg devices can not be found on the current busNumber
    This is only really useful for early Raspberry Pi models!
        """
        self.Print('Loading UltraBorg on bus %d, address %02X' %
        ...
        try:
            ...
            if len(i2cRecv) == I2C_MAX_LEN:
                if i2cRecv[1] == I2C_ID_SERVO_USM:
                    ...
                    self.Print('Found a device at %02X, but it is not a UltraBorg (ID %02X instead of %02X)' % (
...
-0.75 -> 75% to the left
        ...
        powerOut = (float(pwmDuty) - self.PWM_MIN_1) / \
            (self.PWM_MAX_1 - self.PWM_MIN_1)
...
-0.75 -> 75% to the left
        ...
        pwmDuty = (i2cRecv[1] << 8) + i2cRecv[2]
        powerOut = (float(pwmDuty) - self.PWM_MIN_2) / \
            (self.PWM_MAX_2 - self.PWM_MIN_2)
        return (2.0 * powerOut) - 1.0
...
-0.75 -> 75% to the left
        ...
        pwmDuty = (i2cRecv[1] << 8) + i2cRecv[2]
        powerOut = (float(pwmDuty) - self.PWM_MIN_3) / \
            (self.PWM_MAX_3 - self.PWM_MIN_3)
        return (2.0 * powerOut) - 1.0
...
-0.75 -> 75% to the left
        ...
        pwmDuty = (i2cRecv[1] << 8) + i2cRecv[2]
        powerOut = (float(pwmDuty) - self.PWM_MIN_4) / \
            (self.PWM_MAX_4 - self.PWM_MIN_4)
        return (2.0 * powerOut) - 1.0
...
-0.75 -> 75% to the left
        ...
        pwmDuty = int(
            (powerOut * (self.PWM_MAX_1 - self.PWM_MIN_1)) + self.PWM_MIN_1)
...
-0.75 -> 75% to the left
        ...
        powerOut = (position + 1.0) / 2.0
        pwmDuty = int(
            (powerOut * (self.PWM_MAX_2 - self.PWM_MIN_2)) + self.PWM_MIN_2)
        pwmDutyLow = pwmDuty & 0xFF
        pwmDutyHigh = (pwmDuty >> 8) & 0xFF
...
-0.75 -> 75% to the left
        ...
        powerOut = (position + 1.0) / 2.0
        pwmDuty = int(
            (powerOut * (self.PWM_MAX_3 - self.PWM_MIN_3)) + self.PWM_MIN_3)
        pwmDutyLow = pwmDuty & 0xFF
        pwmDutyHigh = (pwmDuty >> 8) & 0xFF
...
-0.75 -> 75% to the left
        ...
        powerOut = (position + 1.0) / 2.0
        pwmDuty = int(
            (powerOut * (self.PWM_MAX_4 - self.PWM_MIN_4)) + self.PWM_MIN_4)
        pwmDutyLow = pwmDuty & 0xFF
        pwmDutyHigh = (pwmDuty >> 8) & 0xFF
...
5000  -> 2.5 ms servo burst, higher than typical longest burst, ~ 7.6% duty cycle
        ...
        if not inRange:
            print('Servo #1 startup position %d is outside the limits of %d to %d' % (
...
5000  -> 2.5 ms servo burst, higher than typical longest burst, ~ 7.6% duty cycle
        ...
        if not inRange:
            print('Servo #2 startup position %d is outside the limits of %d to %d' % (
...
5000  -> 2.5 ms servo burst, higher than typical longest burst, ~ 7.6% duty cycle
        ...
        if not inRange:
            print('Servo #3 startup position %d is outside the limits of %d to %d' % (
...
5000  -> 2.5 ms servo burst, higher than typical longest burst, ~ 7.6% duty cycle
        ...
        if not inRange:
            print('Servo #4 startup position %d is outside the limits of %d to %d' % (
...
Displays the names and descriptions of the various functions and settings provided
        """
        funcList = [UltraBorg.__dict__.get(a) for a in dir(
            UltraBorg) if isinstance(UltraBorg.__dict__.get(a), types.FunctionType)]
